...
export const GET = async () => {
  ...
  await db
    ...
    .values(devs)
    .onConflictDoUpdate({
      target: developers.email,
      set: {
        name: sql`excluded.name`,
        bio: sql`excluded.bio`,
        date_of_starting_working: sql`excluded.date_of_starting_working`,
        daily_rate: sql`excluded.daily_rate`,
      },
    });

  return new Response("Developers inserted or updated");
};
